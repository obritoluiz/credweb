...
begin
  if slctLinha.Text = 'AGN - AGRO' then
    ShowMessage('<b>ATENÇÃO</b>' + #13 + #13 +
                'Esta linha de financiamento se enquadra às pessoas físicas que' +
                ...
                'associação, sindicato rural, ou outra entidade que seja credenciada' +
                'pela Secretaria de Estado do Desenvolvimento Rural e da Agricultura' +
                'Familiar – SEDRAF e que apresentem DAP — Declaração de Aptidão ao PRONAF');
end;
...
begin
  ...
  if (slctLinha.Text = 'MICROCRÉDITO - MEI') or
     (slctLinha.Text = 'MICROCRÉDITO - TURISMO') or
     (slctLinha.Text = 'MICROCRÉDITO - ESCOLAR') then
  begin
    if edtCredPret.Value > 12000  then
    begin
      ShowMessage('<b>Não é possível</b> solicitar mais de 12 mil reais para linha ' + slctLinha.Text);
      slctLinha.SetFocus;
      erro := True;
    end;
  end;
  ...
  begin
    ...
    begin
      ShowMessage('Se você é MEI <b>seu faturamento não pode ser superior a 81 mil reais por ano</b>');
      edtFatAnual.SetFocus;
      erro := True;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      else if SelectTipoFinanc.ItemIndex = 1 then //Financiamento solidário
      begin
    ...
    begin
      ...
      if (frmPrincipal.ConfirmaParticipacao <> 'SIM') then //Se for cliente novo ou não confirma participação no grupo
      begin
    ...
    if SelectTipoFinanc.ItemIndex = 1 then
      ShowMessage('<b>Aviso:</b>' + #13 + #13 +
                  'Seu pré cadastro só será analisado quando todos os integrantes ' +
                  'do seu grupo solidário se cadastrarem e confirmarem a participação no grupo.' +
                  #13 + 'Só é permitido grupo solidário em que todos os integrantes sejam da mesma cidade');
